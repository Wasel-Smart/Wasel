...
const authSecurity_1 = require("../middleware/authSecurity");
...
if (!supabaseClient) {
    errorHandler_1.globalErrorHandler.handleError(new Error('Supabase client not initialized'), errorHandler_1.ErrorSeverity.CRITICAL, { component: 'api', action: 'initialization' });
...
async function getAuthDetails() {
    try {
        ...
        if (error) {
            errorHandler_1.globalErrorHandler.handleError(error, errorHandler_1.ErrorSeverity.HIGH, {
                component: 'api',
...
exports.authAPI = {
    async signUp(email, password, firstName, lastName, phone) {
        try {
            // Input validation
            if (!authSecurity_1.validateInput.email(email)) {
                throw new Error('Invalid email format');
            }
            if (!authSecurity_1.validateInput.password(password)) {
            ...
            // Rate limiting
            if (!(0, authSecurity_1.rateLimit)(`signup_${email}`, 3, 3600000)) {
                throw new Error('Too many signup attempts. Try again in 1 hour.');
            }
            // Sanitize inputs
            const sanitizedFirstName = authSecurity_1.validateInput.sanitize(firstName);
            const sanitizedLastName = authSecurity_1.validateInput.sanitize(lastName || '');
            const sanitizedPhone = authSecurity_1.validateInput.sanitize(phone || '');
    ...
    async signIn(email, password) {
        try {
            ...
            // Rate limiting
            if (!(0, authSecurity_1.rateLimit)(`signin_${email}`, 5, 900000)) {
            ...
            if (error) {
                errorHandler_1.globalErrorHandler.handleError(error, errorHandler_1.ErrorSeverity.MEDIUM, {
                    component: 'api',
                    action: 'signIn',
                    metadata: { email }
                });
...
exports.tripsAPI = {
    async createTrip(tripData) {
        try {
            ...
            const sanitizedData = {
                ...tripData,
                from_location: authSecurity_1.validateInput.sanitize(tripData.from_location),
                to_location: authSecurity_1.validateInput.sanitize(tripData.to_location),
                notes: tripData.notes ? authSecurity_1.validateInput.sanitize(tripData.notes) : null
            };
